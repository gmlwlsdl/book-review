...
**"잉여 속성 체크"** 와 **"할당 가능 검사"** 는 별도의 과정이다. 타입스크립트는 타입을 검사할 때 상황에 따라 다른 검사를 수행한다.

### 1. **할당 가능 검사 (Assignability Check) - 기본 규칙**

타입스크립트의 기본적인 구조적 타이핑 규칙이다. <br />
"A 타입의 값이 B 타입으로 취급될 수 있는가?"를 확인한다. (== B 타입이 요구하는 최소한의 속성을 A가 가지고 있는가?)

예시로, 변수에 객체를 **먼저 할당**한 후 그 변수를 함수에 전달하는 경우가 있다.
...
const myOptions = {
  title: 'My App',
  darkmode: true, // 'darkMode'의 오타
  version: '1.0.0', // Options에 없는 속성
}

// 2. 변수를 함수에 전달합니다.
createWindow(myOptions)

/* 정상이다.
myOptions는 Options가 요구하는 속성을 모두 가지고 있기 때문이다.
myOptions의 추가 속성은 version은 할당 가능 검사에서 무시된다. */
```

### 2. **잉여 속성 검사 (Excess Property Check) - 특별 규칙**

**객체 리터럴(`{...}`)**을 변수나 함수 매개변수에 **직접** 할당할 때만 동작하는 규칙이다. <br />
이 규칙은 개발자의 오타나 실수를 방지하기 위한 것이다.

예시로, 객체 리터럴을 함수에 직접 전달하는 경우가 있다.
...
// 객체 리터럴을 함수에 직접 전달합니다.
createWindow({
  title: 'My App',
  darkmode: true, // 오류다. 'darkmode'는 Options에 없는 속성이기 때문이다.
  version: '1.0.0', // 오류다. 'version'도 Options에 없는 속성이기 때문이다.
})
```

따라서, 잉여 속성 검사를 사용하면 기본적으로 타입 시스템의 구조적 본질을 해치지 않으면서, 객체 리터럴에 알 수 없는 속성은 하용하지 않는 두 가지 목적을 모두 이룰 수 있다. (그래서 '엄격한 객체 리터럴 체크'라고도 불린다고 한다.)

> ⚠️ **잉여 속성 검사는 타입 단언문에서 동작하지 않는다.**
>
> > ### 이것이 타입 단언문보다 선언문을 사용해야 하는 이유 중 하나이다. 💡
> >
> > ![타입 단언문에서의 잉여 속성 체크](image-8.png)

> **잉여 속성 검사를 원하지 않는다면 "인덱스 시그니처"를 사용하자**
>
> > 인덱스 시그니처(`[key: string]: unknown`)는 "이 타입에 내가 모르는 추가 속성이 들어올 수 있어"라고 타입스크립트에게 미리 알려주는 신호이다.
> >
> > ```ts
> > interface Options {
> >   darkMode?: boolean
> >   /* "이 외에 문자열을 키로 하고, 어떤 값이든 가질 수 있는
> >   추가 속성이 더 있을 수 있습니다." 라고 알려준다.
> >   이때 'otherOptions'는 단순히 '키'를 의미하므로, 어떤 이름이든 상관없다. */
...
> > ```
> >
> > ⚠️ 다만 오타를 잡지 못하게 되는 Trade-off가 있다.

### 3. **공통 속성 검사 (Common Property Check) - 특별 검사**

모든 속성이 선택적인(`?`) 타입, 즉 **약한 타입(Weak Type)**에 대해서 수행되는 검사이다.
...
구조적 타이핑 관점에서 `LineChartOptions`는 빈 객체 `{}`도 포함할 수 있을 만큼 범위가 매우 넓다. 이런 타입의 허점을 보완하기 위해 타입스크립트는 **"공통 속성 검사"** 를 수행한다.

이 검사는 할당하려는 값과 약한 타입 사이에 **공통된 속성이 하나라도 있는지** 확인한다. 만약 공통된 속성이 없다면, 타입스크립트는 이를 오타로 간주하고 오류를 발생시킨다.

```ts
const opts = { logScale: true } // 'logscale'의 오타
const o: LineChartOptions = opts
/* 오류이다.
'{ logScale: boolean; }' 유형에 'LineChartOptions' 유형과 공통적인 속성이 없습니다. */
```

위 코드에서 `opts` 객체의 속성(`logScale`)과 `LineChartOptions`의 속성들(`logscale`, `invertedYAxis`, `areaChart`) 사이에는 겹치는 것이 하나도 없기 때문에 타입스크립트는 `logScale`이 `logscale`의 오타일 가능성이 높다고 판단하여 오류를 알려준다.

### 요약: 3가지 타입 검사 규칙 비교

| 구분               | **할당 가능 검사 (기본)** | **잉여 속성 체크 (특별 1)**              | **공통 속성 체크 (특별 2)**                   |
| :----------------- | :------------------------ | :--------------------------------------- | :-------------------------------------------- |
| **언제?**          | 모든 타입 할당 시         | **객체 리터럴 `{...}`을 직접 할당**할 때 | **"약한 타입"** (모든 속성이 `?`)에 할당할 때 |
| **무엇을?**        | 필요한 최소 속성이 있는지 | 정의되지 않은 속성이 있는지              | **겹치는 속성**이 하나라도 있는지             |
| **목적**           | 구조적 호환성 보장        | 오타 방지                                | 약한 타입에서의 오타 방지                     |
| **추가 속성 처리** | 무시하고 통과             | 오류 발생                                | 공통 속성이 있다면 무시하고 통과              |
